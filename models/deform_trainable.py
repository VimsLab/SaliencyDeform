...
class DeformTrainable(Layer):
    # def build(self, input_shape):
    #     self.degree = self.add_weight(shape=(input_shape[0] * input_shape[3],), initializer = 'Zeros', trainable = True, name='w')

    def build(self, input_shape):
        self.saliency_maps = self.add_weight(shape=(input_shape[0], 25, 25, input_shape[3]), initializer = 'random_uniform', trainable = True, name='w')
        super(DeformTrainable, self).build(input_shape)
    ...
    def call(self, activation_maps):
        ...
        # tf.print(self.saliency_maps[0,:,:,0])
        # tf.print('#'*30)

        # saliency_maps = tf.math.divide_no_nan(
        #    tf.math.subtract(
        #       self.saliency_maps ,
        #       tf.math.reduce_min(self.saliency_maps )
        #    ),
        #    tf.subtract(
        #       tf.math.reduce_max(self.saliency_maps ),
        #       tf.math.reduce_min(self.saliency_maps )
        #    )
        # )

        # saliency_maps = self.saliency_maps / tf.norm(self.saliency_maps)
        # tf.print(saliency_maps[0,:,:,0])
        # tf.print('*'*30)

        # saliency_maps = compute_spectral_saliency_4d(activation_maps)
        x_grids, y_grids = create_grid_4d(self.saliency_maps)
        x_grids = tf.image.resize(x_grids, [activation_maps.shape[1]//2,activation_maps.shape[2]//2] , method= tf.image.ResizeMethod.BILINEAR)
        y_grids = tf.image.resize(y_grids, [activation_maps.shape[1]//2,activation_maps.shape[2]//2] , method= tf.image.ResizeMethod.BILINEAR)
        ...
        activation_maps_out = tf.reshape(activation_maps_out, [activation_maps.shape[0], activation_maps.shape[3],activation_maps_out.shape[1], activation_maps_out.shape[2]])
        activation_maps_out = tf.transpose(activation_maps_out,[0,2,3,1])
...
def compute_spectral_saliency_4d(activation_map_inp):
    ...
    activation_map = tf.image.resize(activation_map_inp , [25,25])
    ...
    fft_activation_map = tf.signal.fft2d(tf.cast(activation_map, tf.complex64))
